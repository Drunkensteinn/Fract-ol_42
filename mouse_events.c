...
/*   Updated: 2019/01/23 21:06:05 by ablizniu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				mouse_zoom(int button, int x, int y, t_mlx *mlx)
{
	...
	fractal->mouse.re = (double)x / (mlx->w /
			(fractal->max.re - fractal->min.re)) + fractal->min.re;
	fractal->mouse.im = ((double)y / (mlx->h /
			(fractal->max.im - fractal->min.im)) * -1 + fractal->max.im);
...
int				mouse_events(int x, int y, t_mlx *mlx)
{
	...
	fractal = mlx->scr->fractal_to_draw;
	if (fractal->mouse_block && ((ft_strequ("Julia", fractal->name)
	|| fractal->id == julia) || (ft_strequ("Julia x4", fractal->name)
	|| fractal->id == julia_x_4)))
	{
		fractal->julias_const.im =
				(y - (double)HEIGHT / 2) * (1.0 / (HEIGHT / 2.0));
		fractal->julias_const.re =
				(x - (double)HEIGHT / 2) * (1.0 / (HEIGHT / 2.0));
		refresh(mlx);
	}
	return (0);
}
