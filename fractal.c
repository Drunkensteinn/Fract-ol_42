...
/*   Updated: 2019/01/23 21:36:20 by ablizniu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				t_complex (*func)(t_fractal *), int32_t *color)
{
	...
	while (i < fractal->max_iters)
	{
		...
		put_image(mlx, (int)fractal->x_start,
				(int)fractal->y_start, *color);
		i++;
	}
	if (fractal->in_set == true)
		put_image(mlx, (int)fractal->x_start,
				(int)fractal->y_start, COLOR_BLACK);
}
...
inline void		*fractal(t_fractal *fractal)
{
	...
	{
		fractal->x_start = 0;
		fractal->c.im = fractal->max.im -
				fractal->y_start * fractal->factor.im;
		while (fractal->x_start < fractal->x_end)
		{
			fractal->c.re = fractal->min.re +
					fractal->x_start * fractal->factor.re;
			fractal->in_set = true;
